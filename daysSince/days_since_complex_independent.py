...
def main(inp):
    ...
    if len(inp) == 10:
        # parse the input using regex and assign each number to a variable
        try:
            groups = re.match('([0-9]{2})-([0-9]{2})-([0-9]{4})', inp)
            year = int(groups.group(3))
            month = int(groups.group(1))
            day = int(groups.group(2))
            # try to create a datetime object out of the information provided
            then = datetime(year=year,month=month,day=day).timestamp()
        # if not, print the exception
        except Exception as e:
            print(str(e))
            print(traceback.format_exc())
    # same story here but with hours, minutes, and seconds
    elif len(inp) == 19:
        # parse the input using regex and assign each number to a variable
        try:
            groups = re.match('([0-9]{2})-([0-9]{2})-([0-9]{4}) ([0-9]{2}):([0-9]{2}):([0-9]{2})', inp)
            year = int(groups.group(3))
            month = int(groups.group(1))
            day = int(groups.group(2))
            hours = int(groups.group(4))
            minutes = int(groups.group(5))
            seconds = int(groups.group(6))
            # below are some debugging statements i've left in to show how i typically debug while developing simple scripts like this
            # print(f'year is {year} with type {type(year)}')
            # print(f'month is {month} with type {type(month)}')
            # print(f'day is {day} with type {type(day)}')
            # print(f'hours is {hours} with type {type(hours)}')
            # print(f'minutes is {minutes} with type {type(minutes)}')
            # print(f'seconds is {seconds} with type {type(seconds)}')
            # try to create a datetime object out of the information provided
            then = datetime(year=year,month=month,day=day,hour=hours,minute=minutes,second=seconds).timestamp()
        # if not, print the exception
        except Exception as e:
            print(str(e))
            print(traceback.format_exc())
        ...
        # parse the input using regex and assign each number to a variable
        try:
            groups = re.match('([0-9]{2})-([0-9]{2})-([0-9]{4}) ([0-9]{2}):([0-9]{2}):([0-9]{2})\.([0-9]*)', inp)
            year = int(groups.group(3))
            month = int(groups.group(1))
            day = int(groups.group(2))
            hours = int(groups.group(4))
            minutes = int(groups.group(5))
            seconds = int(groups.group(6))
            microseconds = int(groups.group(7))
            # try to create a datetime object out of the information provided
            then = datetime(year=year,month=month,day=day,hour=hours,minute=minutes,second=seconds,microsecond=microseconds).timestamp()
        # if not, print the exception
        except Exception as e:
            print(str(e))
            print(traceback.format_exc())
    else:
        print(f'Invalid input length. Should be:\n10 for MM-DD-YYYY\n19 for MM-DD-YYYY HH:MM:SS\n26 for MM-DD-YYYY HH:MM:SS.XXXXXX\nDid you forget to pad your microseconds?')
        quit()
...
if __name__ == "__main__":
    print(f'Enter your starting date. Correct formatting options are:')
    print(f'MM-DD-YYYY')
    print(f'MM-DD-YYYY HH:MM:SS')
    print(f'MM-DD-YYYY HH:MM:SS.XXXXXX')
    main(input("> "))